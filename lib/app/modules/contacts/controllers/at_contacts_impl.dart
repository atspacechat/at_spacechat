...
class AtContactsImpl implements AtContactsLibrary {
  ...
  Future<List<AtContact>> listContacts() async {
    ...
    var regex =
        '${AppConstants.CONTACT_KEY_PREFIX}.*.${atSign}.${AppConstants.LIBRARY_NAMESPACE}$appNamespace'
    ...
    print("scanlist "+scanList.toString());
    ...
    for (var key in scanList) {
      ...
      if(atKey.sharedBy != null && !atKey.sharedBy!.contains("@")) {
        atKey.sharedBy = "@"+ atKey.sharedBy.toString();
      }
  ...
  AtKey _formKey(KeyType keyType,
    ...
    var atKey;
    // try{
    //   atKey = _formAtKey(modifiedKey, isOld: isOld);
    // }on Exception {
      atKey = _formAtKey(modifiedKey);
    // }
    return atKey;
  }
